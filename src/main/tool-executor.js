...
const { TOOL_HANDLERS: COMPUTER_USE_HANDLERS } = require('./computer-use');
...
function bash(input, ctx) {
  ...
  return new Promise((resolve) => {
    ...
    proc.on('close', (code) => {
      ...
      // 记录 Bash 执行结果到日志
      const logPrefix = `[Bash] exit=${code}`;
      if (code === 0) {
        console.log(`${logPrefix} ${command.substring(0, 100)}${command.length > 100 ? '...' : ''}`);
        if (stderr) console.log(`${logPrefix} stderr: ${stderr.substring(0, 500)}`);
      } else {
        console.error(`${logPrefix} FAILED: ${command.substring(0, 100)}`);
        if (stdout) console.error(`${logPrefix} stdout: ${stdout.substring(0, 500)}`);
        if (stderr) console.error(`${logPrefix} stderr: ${stderr.substring(0, 500)}`);
      }
...
module.exports = { executeTool, taskStore, taskGetAll, taskClearAll, taskCompleteAll, readTextSmart, setEnvConfig, clearBackgroundAgents, detectWinShell };
