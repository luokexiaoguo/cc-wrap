  ...
  async function walk(current, depth) {
    ...
    for (const item of items) {
      ...
      if (item.isDirectory()) {
        ...
        files.push({ name: item.name, path: fullPath, type: 'file', size, mtime });
      }
    }
  }
  await walk(dir, 0);
...
app.whenReady().then(() => {
  ...
  // ========== Computer Use 预览截图 ==========
  ipcMain.handle('computer-use-preview', async () => {
    try {
      const { captureScreen } = require('./computer-use/screenshot');
      const result = await captureScreen({ quality: 60 });
      return { success: true, image: result.image, width: result.width, height: result.height };
    } catch (err) {
      return { success: false, error: err.message };
    }
  });
