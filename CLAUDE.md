...
19 built-in tools defined in `tools.js` with Anthropic-format `input_schema`: Read, Write, Edit, Glob, Grep, Bash, ListDirectory, WebSearch, WebFetch, Agent, GetAgentResult, TaskCreate, TaskUpdate, InstallSkill, InstallMcp, DiscoverMcp, AskUserQuestion, ReadImage, DeleteMemory.
...
**DiscoverMcp**: Scans the local system for existing MCP server configurations. Sources: `claude-desktop` (`%APPDATA%/Claude/claude_desktop_config.json`), `npm` (global npm packages matching MCP patterns), `pip` (Python MCP packages), `cc-wrap` (current `mcp-servers.json`), `path` (known MCP CLIs like `mmx`, `uvx`). Returns structured report with server names, commands, and types — AI can then import via InstallMcp. Supports `import: true` parameter to auto-import discovered servers (skips duplicates by name or command match).

**ReadImage**: Reads an image file and returns the file path + MIME type to the renderer. The renderer asynchronously loads the image via `read-file-as-data-url` IPC (not base64 over IPC — too large). Handles Git Bash path conversion (`/e/...` → `E:\...`). Images are rendered in the assistant message body (not collapsed inside tool call cards).

**DeleteMemory**: Deletes a memory by exact content match or index from `memory.json`. Confirms with user before deleting.
...
**Image Display**: System prompt tells AI to use `ReadImage` for inline image display. MCP image tools auto-render. URLs with image extensions auto-render. Images from tool calls appear in message body (not collapsed in tool cards). Async loading: renderer receives file path, loads via `read-file-as-data-url` IPC, shows loading indicator then image.

**Git Bash path normalization**: Bash tool on Windows uses Git Bash which generates Unix-style paths (`/e/openclawPJ/...`). These must be converted to Windows paths (`E:\openclawPJ\...`) before passing to `fs` APIs. Conversion happens in `read-file-as-data-url` handler and `loadToolResultImages()`.
